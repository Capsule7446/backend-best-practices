...
2. 建运行工作区 `<workdir>/`，写入 `_manifest.md`。
3. 先识别耦合和适配度，再设计目标读模型。
...
| 04 | cqrs-read-model-design | `03-aggregation-view.md`,`01-domain-read-decoupling.md` | `04-read-model-design.md` | |
...
| 门禁 | 位置 | 放行条件 |
| :-- | :-- | :-- |
| G0 适配 | `02-fit-check.md` 后 | 简单 DTO/Query Service 足够时不升级；需要 Read Model 时有明确驱动 |
