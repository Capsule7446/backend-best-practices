...
电商系统支持 Stripe 与 Adyen 两套支付产品族：PaymentClient、RefundClient、WebhookVerifier 必须来自同一厂商。

## 适配判断

- 使用：存在多个产品族，且产品族内对象必须配套变化，例如支付渠道、UI 主题、云厂商 SDK。
- 避免：只有一个产品或产品族不会整体变化时不要使用。
...
final class ConcreteVariation implements StableRole {
    public Result execute(Request request) {
        // implement Abstract Factory variation for the business case
        return Result.ok();
    }
}
