...
description: "GoF Abstract Factory（抽象工厂）设计模式能力。用于在 存在多个产品族，且产品族内对象必须配套变化，例如支付渠道、UI 主题、云厂商 SDK。 时，输出语言无关思想、适用场景、最佳实践范式、多语言实现差异和案例。"
...
该 Skill 同时承担固定模式入口：将输入交给 `workflow-design-pattern` 编排，并将候选模式固定为 `abstract-factory`。
...
使用 抽象工厂 模式解决这类问题：创建一组相关或相互依赖的对象，而不暴露具体产品类。
...
- 存在多个产品族，且产品族内对象必须配套变化，例如支付渠道、UI 主题、云厂商 SDK。
- 变化轴明确，且简单函数、组合或语言原生机制不足以保持清晰。
- 需要让新增变体的成本低于修改既有调用方的成本。

## 不适用场景

- 只有一个产品或产品族不会整体变化时不要使用。
...
- 先定义产品族接口，再定义工厂接口；让业务只依赖抽象产品和抽象工厂；把产品族选择放在组合根或配置层。
