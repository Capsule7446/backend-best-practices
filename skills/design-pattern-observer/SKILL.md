...
该 Skill 同时承担固定模式入口：将输入交给 `workflow-design-pattern` 编排，并将候选模式固定为 `observer`。
...
使用 观察者 模式解决这类问题：对象状态变化时通知多个依赖者。
