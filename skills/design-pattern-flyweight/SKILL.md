...
该 Skill 同时承担固定模式入口：将输入交给 `workflow-design-pattern` 编排，并将候选模式固定为 `flyweight`。
...
使用 享元 模式解决这类问题：共享大量细粒度不可变对象，减少内存占用。
