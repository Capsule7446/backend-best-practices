...
该 Skill 同时承担固定模式入口：将输入交给 `workflow-design-pattern` 编排，并将候选模式固定为 `mediator`。
...
使用 中介者 模式解决这类问题：用中介对象封装多个对象间的复杂交互。
