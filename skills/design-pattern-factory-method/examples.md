...
通知发送流程固定，但根据 channel 创建 EmailSender、SmsSender、PushSender。

## 适配判断

- 使用：框架固定流程，但某一步需要由扩展方决定具体对象。
- 避免：创建逻辑没有扩展点或只是 new 的别名时不要使用。
...
final class ConcreteVariation implements StableRole {
    public Result execute(Request request) {
        // implement Factory Method variation for the business case
        return Result.ok();
    }
}
