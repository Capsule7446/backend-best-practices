...
PaymentGateway 外层叠加 Retry、Metrics、Idempotency 三个装饰器。

## 适配判断

- 使用：缓存、重试、限流、日志、权限、压缩等横切增强可组合。
- 避免：增强顺序含糊或调用链难调试时谨慎使用。
...
final class ConcreteVariation implements StableRole {
    public Result execute(Request request) {
        // implement Decorator variation for the business case
        return Result.ok();
    }
}
