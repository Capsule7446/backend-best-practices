# Chain of Responsibility 案例

## 业务案例

订单提交前依次经过库存、风控、价格、优惠券校验。

## 适配判断

- 使用：校验链、过滤器、中间件、审批流、规则流水线。
- 避免：处理顺序不重要或结果需要强一致汇总时不要滥用。
...
final class ConcreteVariation implements StableRole {
    public Result execute(Request request) {
        // implement Chain of Responsibility variation for the business case
        return Result.ok();
    }
}
