...
该 Skill 同时承担固定模式入口：将输入交给 `workflow-design-pattern` 编排，并将候选模式固定为 `bridge`。
...
使用 桥接 模式解决这类问题：把抽象与实现分离，让两个维度独立变化。
