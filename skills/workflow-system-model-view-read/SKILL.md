...
description: 执行 workflow-system-model-view-read 编排流程；这是由 Command 或其他 Skill 调用的注册 Workflow。
---

## 做什么

执行 `workflow-system-model-view-read` 的完整编排流程，负责阶段顺序、输入输出交接、门禁和回溯。
