...
SKILL.md 是**默认会被加载进 context 的核心**，所以只保留"技能本身的描述"（上面四段）。示例、深度参考、长走查等**拆到同目录附加文件**（如 `examples.md`、`reference.md`）；跨能力共享的参考（如语言剖面）放**插件根 `references/`**。SKILL.md 只用一行**指路**且必须写明位置（"同目录"或"插件根 references/"），由 AI 在**需要时**才去读——渐进披露，避免一次性把所有东西塞进 context。
...
链中环节确实不适用时（如无任何订阅方的领域事件、全 `avoid` 时的 RM），以显式 `n/a(理由)` 记录即视为闭合——**缺席必须显式声明，不得留空**。
