...
| 11 | adapter-impl | **[已废弃→0.3.0 移除]** 兼容入口 | 接口骨架 + 契约语义 + 剖面 | （改用 52-55 的分层实现能力）|
...
- 当前能力总数 **58**（DDD 15 + CQRS 6 + Design Pattern 26 + Application 4 + 分层实现 4 + 读侧交付 2 + 模式扫描 1；其中 adapter-impl 为废弃兼容入口）。
