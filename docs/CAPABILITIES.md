...
| 23 | design-pattern-implementation | 将模式转成语言无关蓝图并生成目标语言惯用实现 | 模式决策 + 业务角色 + 目标语言 + 约束 + 现有接口 | 蓝图、角色映射、目标语言实现、测试点、演进注意 |
| 24 | design-pattern-review | 审查模式是否必要、角色是否清楚、实现是否惯用 | 模式方案/代码 + 目标语言 + 设计目标 + 测试 | pass/needs_changes/fail、分级发现、更简单替代方案、修正建议 |
