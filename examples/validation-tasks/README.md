...
1. 按任务文件标注的入口启动（如 `/backend-best-practices:ddd-new <任务输入>`）。
...
| # | 任务 | 入口 | 核心考点 |
| :-- | :-- | :-- | :-- |
| 01 | 简单 CRUD（书签管理）| ddd-new | 有 Application + 查询契约，但拒绝 CQRS/Saga/GoF |
| 02 | 复杂订单 | ddd-new | Aggregate、编排、Outbox、业务视图、权限齐备 |
| 03 | 跨上下文 Dashboard | cqrs-read-model-new | 选 Read Model、字段来源、新鲜度、重建、权限 |
| 04 | Brownfield 查询迁移 | cqrs-read-model-refactor | 特征化/Parity Diff、Backfill、切换与回滚 |
